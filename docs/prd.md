...
solving capabilities of candidates applying for Full-Stack Engineering roles at 101 Digital PTE LTD.

#### 1.1 Approach

Prior to commencing the mini-project, candidates must thoroughly review and understand the
following:
...
Candidates are required to design and develop a full-stack web application named SimpleInvoice. The
application must comprise a ReactJS frontend and a NestJS backend, supported by a database
designed and implemented by the candidate.

No external third-party APIs are required for this project. Candidates are expected to develop and
manage the complete application stack, including the frontend, backend, and database layers.
...
Candidates must implement a secure authentication mechanism that satisfies the following
requirements:
...
Users must be able to view the details of an individual invoice by selecting an invoice from the Invoice
List screen.

Upon selection, the application shall navigate the user to the Invoice Details view, which must display
comprehensive information relating to the selected invoice, including:
...
All invoice data presented in the detail view must accurately reflect the corresponding record stored
in the system.
...
The backend service must be implemented using NestJS with TypeScript, and must expose a RESTful
API.

The application must be supported by a relational database, with PostgreSQL recommended as the
preferred database system.

The backend implementation should ensure a clean, modular architecture and support reliable data
persistence and retrieval for all application features.
...
Overdue is a derived status — it is not persisted in the database. The backend computes it at read
time:
...
The backend must implement authentication and authorization using JSON Web Tokens (JWT) to
support stateless session management.
...
##### threshold is enforced, candidates are expected to ensure adequate coverage of core
...
- At least one integration or end-to-end test must be implemented, covering a

##### complete key workflow (for example: creating an invoice and verifying its appearance

##### in the invoice list).

##### 2.3.8 API Documentation

##### The backend API must be documented using Swagger/OpenAPI, generated via the
...
Organize your project as a _monorepo_ or two _separate_ repos. Either is acceptable — document your
...
##### The application must support environment-based configuration using .env files for all

##### environment-specific settings.

##### The following requirements apply:

- All environment-specific configuration values (including database connection strings,

##### JWT secrets, and application ports) must be managed via .env files.

- A .env.example file must be provided, containing all required configuration keys without

##### any real or sensitive values.

- All secrets, credentials, and configuration parameters must be sourced exclusively
...
status (^) enum Persisted: Draft, Pending, Paid. Overdue is derived at
read time.
...
time.
All submission materials, including links to code repositories, must be sent to the designated submission contact
provided below.
...
Paid), varied invoice dates, due dates, amounts, and customer names so that all list
